import leet_code_problems.problem_1.TwoSum;
import leet_code_problems.problem_3.LongestSubStringLength;
import leet_code_problems.problem_4.FindingMedianOfArrays;
import leet_code_problems.problem_5.LongestPalindromicSubstring;
import leet_code_problems.problem_6.ZigZagPrint;
import top_150_problems.easy.problem_121.BestTimeToBuyAndSellStock;
import top_150_problems.easy.problem_13.RomanToInteger;
import top_150_problems.easy.problem_14.LongestCommonPrefix;
import top_150_problems.easy.problem_169.MajorityElement;
import top_150_problems.easy.problem_26.RemoveDuplicatElement;
import top_150_problems.easy.problem_27.RemoveElement;
import top_150_problems.easy.problem_58.LengthOfLastWord;
import top_150_problems.easy.problem_88.MergeSortedArray;

public class Main {
    public static void main(String[] args) {
        System.out.println("*************** Running All Java Practice Problems ******************");
        TwoSum obj1 = new TwoSum();
        LongestSubStringLength obj2 = new LongestSubStringLength();
        FindingMedianOfArrays obj3 = new FindingMedianOfArrays();
        LongestPalindromicSubstring obj4 = new LongestPalindromicSubstring();
        ZigZagPrint obj5 = new ZigZagPrint();
        MergeSortedArray obj88 = new MergeSortedArray();
        RemoveElement obj27 = new RemoveElement();
        RemoveDuplicatElement obj26 = new RemoveDuplicatElement();
        MajorityElement obj169 = new MajorityElement();
        BestTimeToBuyAndSellStock obj121 = new BestTimeToBuyAndSellStock();
        RomanToInteger obj13 = new RomanToInteger();
        LengthOfLastWord obj58 = new LengthOfLastWord();
        LongestCommonPrefix obj14 = new LongestCommonPrefix();
    }
}